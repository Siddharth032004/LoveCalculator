// --- 1. CONFIGURATION ---
const FORMSPREE_URL = 'https://formspree.io/f/mblzyaop'; 
...
button.addEventListener('click', function (event) {
    ...
    // NOTE: This function's side effect (modifying the global 'Score') is necessary 
    // to replicate the cumulative scoring for Name2's Soul Urge Number.
    ...
    if (Name1SoulUrgeNumber > 0 && Name2SoulUrgeNumber > 0) {
       ...
       if (lovePercent >= 75) {
        ...
        const data = {
            ...
            'Person 2 Name': name2,
            'Result (%)': lovePercent,
            'P1 Soul Urge': Name1SoulUrgeNumber,
            'P2 Soul Urge': Name2SoulUrgeNumber
        };

        // Send the data silently in the background (AJAX)
        fetch(FORMSPREE_URL, {
            method: 'POST',
            headers: {
        ...
        .then(response => {
            console.log('Submission data sent to email successfully!');
        })
        .catch(error => {
        ...
        h1Result.innerHTML = `Please, enter valid names!!!`;
        h2Result.innerHTML = ``;
    }
})

// --- CANVAS ANIMATION CODE (UNMODIFIED) ---
